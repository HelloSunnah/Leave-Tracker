...
Auth::routes();
    ...
    Route::get('/leave/remove/{id}', [LeaveRequestController::class, 'remove_request'])->name('remove.request');

    //user profile 
    Route::get('/user/profile', [HomeController::class, 'user_profile'])->name('user.profile');
    Route::post('/user/password', [HomeController::class, 'user_password'])->name('password.update');
